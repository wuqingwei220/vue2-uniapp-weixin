<template>
   <view>
     <!-- <NavBar title="注册" titleSize="34" titleColor="#000000"  :showBack="true" totalBackground="#C2F21F"></NavBar> -->
	 <view class="header">
      <NavBar
        title="注册"
        titleSize="34"
    ...
    ></view>
				...
				<view class="register_down register_down_verlogin">
                    <view class="addAddress_items">
						<view class="addAddress_item">
                             <view class="decration">用户名</view>
							 <view class="addAddress_iright">
								<input type="text" placeholder-style="color:#C3C4C7;font-size:28rpx;font-weight: normal" placeholder="请输入用户名" v-model="userAddress.name"/>
							 </view>
						</view>
					 </view>
					 <view class="addAddress_items">
						<view class="addAddress_item">
							 ...
							 <view class="addAddress_iright">
								<input type="number" placeholder-style="color:#C3C4C7;font-size:28rpx;font-weight: normal" placeholder="请输入你的手机号码" v-model="userAddress.telephone"/>
							 </view>
						</view>
					 </view>
                      <view class="addAddress_items">
						<view class="addAddress_item">
                             <view class="decration">密码</view>
							 <view class="addAddress_iright">
								<input type="password" placeholder-style="color:#C3C4C7;font-size:28rpx;font-weight: normal" placeholder="请输入密码" v-model="userAddress.password"/>
							 </view>
						</view>
					 </view>
					 <view class="addAddress_items">
						 <view class="addAddress_item">
							...
							<view class="addAddress_yzmleft">
								<input type="number"  placeholder-style="color:#C3C4C7;font-size:28rpx;font-weight: normal" placeholder="请输入验证码" v-model="userAddress.code"/>
							</view>
							<view class="addAddress_yzmright">
								<button class="code-btn"
								@click='getVerificationCode' style="color:#333333"	v-if="ifCode" :disabled='disabled'>{{btntxt}}</button>
								<button
									class="code-btn"
									:disabled="disabled"
									v-else
            ...
            <view class="verlogin_wrap">
				<view class="verlogin_wleft">
                   <image class="tick" v-if="tips" src="http://oss.juxingta.com/static/images/2022-08-12/ea015f57-22be-4516-8083-042ff5b9c9a9___%E7%BB%84%207%402x.png"></image>
					<view class="verlogin_wlview">
						<van-checkbox 
						icon-size="28rpx"
                        checked-color="#BBF100"
                        icon-class="iocn"
						:value="agreeChecked" @change="agree"></van-checkbox>
					</view>
				</view>
				<view class="verlogin_wright">我已阅读并同意<text  @click="privacy">《用户协议》</text><text @click="agreement">《隐私协议》</text></view>
			</view>
            <view class="login-btn">
                <view class="btn-submit" v-if="allInput" @click="enroll">
                    注册
                </view>
...
import {sendChangePhone,shipperCreate,signin} from '@/api/user'
import { handleGetUserInfo } from "@/utils/all-request";
 export default {
    ...
    data() {
        return {
            btntxt: '获取验证码',
            userAddress: {telephone:"",code:"",name:"",password:""},
            //协议
            agreeChecked:false,
			//协议提示img
			tips: false,
	...
	watch:{
	deep:true,
	'userAddress.telephone':async function(newVal,oldVal){
	   try {
	...
	computed: {
    allInput() {
		if (this.userAddress.telephone && this.userAddress.code && this.userAddress.password && this.userAddress.name) {
			return true;
			} else {
				return false;
			}
    ...
    methods:{
        ...
        // 获取验证码
        	async getVerificationCode() {
				let phone = this.userAddress.telephone;
				try {
					await this.$validator({
						phone: [
							required(required.message("手机号")),
				...
				sendChangePhone(postData).then(res => {
			...
			async enroll(){
				...
				let password = this.userAddress.password;
				let telephone = this.userAddress.telephone;
				let code = this.userAddress.code;
				try {
					await this.$validator({
						telephone: [
							required(required.message("手机号")),
						...
						password: [required(required.message("密码"))],
						name: [required(required.message("用户名"))],
						code: [required(required.message("验证码"))],
					}).validate({ telephone ,name ,password,code});
				} catch (e){
                      uni.showToast({
                        title: e.errors[0].message,
                        icon: 'none',
				...
				let postData={
					name,
					code,
					password:this.$md5(this.userAddress.password),
					contacts:name,
					telephone
				}
				shipperCreate(postData).then(res => {
					uni.hideLoading();
					if(res.data.code==200){
						uni.hideLoading();
						this.$Router.replace({
						...
						// 	console.log("请求成功");
						// 	uni.hideLoading();
						// 	let getToken = res1.headers.token;
						// 	this.$store.commit("login", getToken[0], null);
						// 	let getAuthor = res1.data.data.token;
						// 	this.$store.commit("author", getAuthor, null);
						// 	this.$store.commit("setAuthorPhone", telephone, null);
						// 	// 存储用户信息
						// 	handleGetUserInfo();
						// }).catch((err) => {
						// 	uni.hideLoading();
