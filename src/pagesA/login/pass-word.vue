  ...
  <view>
    ...
    <view class="verlogin_wrap">
      ...
      <view class="verlogin_wright"
        >我已阅读并同意<text @click="privacy">《用户协议》</text
        ><text @click="agreement">《隐私协议》</text></view
      >
    </view>
    <view class="login-btn">
      ...
      <view class="changePw">
        <text @click="passCode">验证码登录</text>
...
export default {
  ...
  data() {
    return {
      // userAddress: { phone: "18510939493", password: "123456" },
      userAddress: { phone: "", password: "" },
      //协议
      agreeChecked: false,
      //协议提示img
      tips: false,
  ...
  computed: {
    allInput() {
      if (this.userAddress.phone && this.userAddress.password) {
        return true;
      } else {
        return false;
      }
    ...
    async onSubmit() {
      ...
      try {
        ...
        let phone = _this.userAddress.phone, //联系电话
          password = _this.$md5(_this.userAddress.password); // 密码
