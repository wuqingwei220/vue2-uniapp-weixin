 ...
 * @returns {*}
 */
 export function idNumberMsg(message) {
  return attrs.pattern(
...
bindMessage(idNumberMsg, "%s请输入正确的身份证号");
 ...
 * @returns {*}
 */
export function chs_password(message) {
  return attrs.pattern(/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,16}$/, message);
}
  bindMessage(chs_password, "请输入符合规则的密码");
